...
This one is really easy since we only need a docker image with a mySQL database. However, we need the database to be initialized. If you remember from [DevOping - 2](https://blog.adriapt.com/posts/DevOping-2/), we created a .sql file that was executed when the sql server was initialized. We can also use this script to initialize the database in our docker image. 
